...
この repo では commit 時に formatter を自動実行する hook を使います。clone 直後は hooksPath を設定してください。

```sh
git config core.hooksPath .githooks
```

hook は staged file のうち `.astro` を Prettier、それ以外の対応ファイルを `oxfmt` で整形して再 stage します。同じファイルに unstaged change がある場合は、意図しない混在を避けるため commit を止めます。
...
| `pnpm run format`         | `.astro` は Prettier、それ以外は `oxfmt` で整形します。 |
| `pnpm run format:check`   | 同じ振り分けで format check を実行します。              |
...
この repo の環境変数は `.env.example` を契約として管理します。実際の値は Cloudflare の build variables、ローカルの ignored `.env`、または shell env に置いてください。

Astro build-time variables:
